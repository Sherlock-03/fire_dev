...
#pragma diag_suppress 177 /* warning:  #177-D: variable "pxCtrl"  was declared but never referenced */

/* Includes */
#include <cmsis_os.h>
#include "lib/memory/lib_mem_spi_flash.h"
...
{   
    ...
    case MEM_CTRL_OP_SPIFLASH_ERASE_CHIP:
        xRet = prvChipErase(xHandle);
        break;
    default:
        xRet = status_err;
        break;
...
{
    TRACE("SpiFlashIfEraseWrite INTER\r\n");
...
{   
    ...
    if ((ulAddr < pxCtrl->xConfig.ulStAddr) || ((ulAddr + ulLength) > pxCtrl->xConfig.ulEdAddr)) {
        TRACE("error : get here\r\n");
        return status_err;
    }
    else {
        TRACE("Spi_Flash_Erase_Write run\r\n");
        if (Spi_Flash_Erase_Write(ulAddr, ulLength, pucData) != FLASH_SUCCESS) {
            TRACE("Spi_Flash_Erase_Write run error\r\n");
            ASSERT(0);  /* We should never get here! */
        }
        return status_ok;
    }
