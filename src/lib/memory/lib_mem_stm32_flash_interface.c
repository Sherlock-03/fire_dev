...
/* Includes */
#include "lib/memory/lib_mem_stm32_flash_interface.h"
...
/* Forward declaration */
static uint32_t    prvStm32FlashIfErase         (uint32_t ulStartSector);
static uint32_t    prvStm32FlashIfWriteUint8    (uint32_t ulAddr, uint8_t *pucSrc, uint32_t ulSizeInUint8);
static uint32_t    prvStm32FlashIfWriteUint16   (uint32_t ulAddr, uint16_t *pusSrc, uint32_t ulSizeInUint16);
static uint32_t    prvStm32FlashIfWriteUint32   (uint32_t ulAddr, uint32_t *pulSrc, uint32_t ulSizeInUint32);
static uint32_t    prvStm32FlashIfReadUint8     (uint32_t ulAddr, uint8_t *pucDest, uint32_t ulSizeInUint8);
static uint32_t    prvStm32FlashIfReadUint16    (uint32_t ulAddr, uint16_t *pusDest, uint32_t ulSizeInUint16);
static uint32_t    prvStm32FlashIfReadUint32    (uint32_t ulAddr, uint32_t *pulDest, uint32_t ulSizeInUint32);
static uint32_t    prvStm32FlashIfGetWpStatus   (uint32_t ulObWrpPages);
/* Call HAL_FLASH_OB_Launch to make the config working, HAL_FLASH_OB_Launch will reset the MCU automatically!  */
static uint32_t    prvStm32FlashIfSetWpStatus   (uint32_t ulObWrpPages, uint32_t ulStatus);
static uint32_t    prvWriteUint32               (uint32_t ulAddr, uint32_t *pulSrc, uint32_t ulSizeInUint32);
...
status_t Stm32FlashIfCtrl(MemHandle_t xHandle, MemCtrlOperation_t xCtrlOp, uint32_t ulExtraPara)
{
    ...
    case MEM_CTRL_OP_STM32FLASH_ERASE:
    {
        ...
        xRet = ((FLASHIF_OK == prvStm32FlashIfErase(xCtrl.ulStartSector)) ? status_ok : status_err);
        break;
    }
    case MEM_CTRL_OP_STM32FLASH_WP:
    {
        MemStm32FlashWp_t xCtrl = *(MemStm32FlashWp_t*)ulExtraPara;
        xRet = ((FLASHIF_OK == prvStm32FlashIfSetWpStatus(xCtrl.ulObWrpPages, xCtrl.ulStatus)) ? status_ok : status_err);
        break;
    }
    default:
        xRet = status_err;
...
status_t Stm32FlashIfRead(MemHandle_t xHandle, uint32_t ulAddr, uint32_t ulLength, OUT uint8_t *pucData)
{
    ...
    prvStm32FlashIfReadUint8(ulAddr, pucData, ulLength);
    return status_ok;
}

status_t Stm32FlashIfWrite(MemHandle_t xHandle, uint32_t ulAddr, uint32_t ulLength, IN uint8_t *pucData)
{
    MemCtrl_t *pxCtrl = MEM_GET_CTRL(xHandle);
    ASSERT(NULL != pxCtrl);
    ...
    prvStm32FlashIfWriteUint8(ulAddr, pucData, ulLength);
    return status_ok;
}

uint32_t Stm32FlashIfGetWpStatus(MemHandle_t xHandle, uint32_t ulObWrpPages)
{
    return prvStm32FlashIfGetWpStatus(ulObWrpPages);
}

uint32_t prvStm32FlashIfWriteUint8(uint32_t ulAddr, uint8_t *pucSrc, uint32_t ulSizeInUint8)
{
    uint32_t ulRet = 0;
    ...
    ulRet = prvStm32FlashIfWriteUint32(ulAddr, (uint32_t*)pucSrc, ulSizeInUint32);
    if (FLASHIF_OK != ulRet) {
        return ulRet;
    }
    ...
    if (ulExtraByte) {
        uint16_t n;
        uint8_t ucData[4];
        prvStm32FlashIfReadUint32(ulAddr + ulSizeInUint32 * 4, (uint32_t*)ucData, 1);
        for (n = 0; n < ulExtraByte; n++) {
            ucData[n] = pucSrc[ulSizeInUint32 * 4 + n];
        }
        ulRet = prvStm32FlashIfWriteUint32(ulAddr + ulSizeInUint32 * 4, (uint32_t*)ucData, 1);
    }
...
uint32_t prvStm32FlashIfWriteUint16(uint32_t ulAddr, uint16_t *pusSrc, uint32_t ulSizeInUint16)
{   
    uint32_t ulRet = 0;
    uint32_t ulSizeInUint32 = 0;
    uint32_t ulExtraByte = 0;
    ...
    ulRet = prvStm32FlashIfWriteUint32(ulAddr, (uint32_t*)pusSrc, ulSizeInUint32);
    if (FLASHIF_OK != ulRet) {
        return ulRet;
    }
    ...
    if (ulExtraByte) {
        uint16_t n;
        uint16_t usData[2];
        prvStm32FlashIfReadUint32(ulAddr + ulSizeInUint32 * 4, (uint32_t*)usData, 1);
        for (n = 0; n < ulExtraByte; n++) {
            usData[n] = pusSrc[ulSizeInUint32 * 2 + n];
        }
        ulRet = prvStm32FlashIfWriteUint32(ulAddr + ulSizeInUint32 * 4, (uint32_t*)usData, 1);
    }
    
    return ulRet;
}

uint32_t prvStm32FlashIfWriteUint32(uint32_t ulAddr, uint32_t *pulSrc, uint32_t ulSizeInUint32)
{
    ...
    HAL_FLASH_Unlock();
    while (1)  {    
        prvStm32FlashIfReadUint32((ulSectorNum * STM32_FLASH_PAGE_SIZE + STM32_FLASH_BASE_ADDR), ulFlashBuffer, (STM32_FLASH_PAGE_SIZE / 4));
        ...
        if (i < ulSectorLeft) {
            ...
            prvWriteUint32((ulSectorNum * STM32_FLASH_PAGE_SIZE + STM32_FLASH_BASE_ADDR), ulFlashBuffer, (STM32_FLASH_PAGE_SIZE / 4));
        }
        else {
            prvWriteUint32(ulAddr, pulSrc, ulSectorLeft);
        }
...
uint32_t prvStm32FlashIfReadUint8(uint32_t ulAddr, uint8_t *pucDest, uint32_t ulSizeInUint8)
{   
    uint32_t ulNum = 0;
...
uint32_t prvStm32FlashIfReadUint16(uint32_t ulAddr, uint16_t *pusDest, uint32_t ulSizeInUint16)
{   
    uint32_t ulNum = 0;
    ulAddr = (uint32_t)ulAddr;
...
uint32_t prvStm32FlashIfReadUint32(uint32_t ulAddr, uint32_t *pulDest, uint32_t ulSizeInUint32)
{
    uint32_t ulNum = 0;
    ulAddr = (uint32_t)ulAddr;
...
uint32_t prvStm32FlashIfGetWpStatus(uint32_t ulObWrpPages)
{
...
uint32_t prvStm32FlashIfSetWpStatus(uint32_t ulObWrpPages, uint32_t ulStatus)
{
