...
#include "lib/memory/lib_mem_fram.h"
#include "lib/memory/lib_mem_spi_flash_interface.h"
...
#define MEM_MUTEX_INIT()                                \
    do                                                  \
    {                                                   \
        s_xMemMutex = osMutexNew(NULL);                 \
        if (s_xMemMutex == NULL)                        \
        {                                               \
            /* Handle error */                          \
        }                                               \
    } while (0)
#define MEM_LOCK()   osMutexAcquire(s_xMemMutex, osWaitForever)
#define MEM_UNLOCK() osMutexRelease(s_xMemMutex)
#else
#define MEM_MUTEX_INIT()
#define MEM_LOCK()
#define MEM_UNLOCK()
...
#if MEM_ENABLE_STM32FLASH
    case MEM_DEVICE_STM32FLASH:
        xRet = Stm32FlashIfWrite(xHandle, ulAddr + pxCtrl->xConfig.ulStAddr, ulLength, pucData);
        break;
#endif /* MEM_ENABLE_STM32FLASH */
    default:
        ASSERT(0); /* We should never get here! */
...
status_t MemShow(MemHandle_t xHandle)
{
    ...
    TRACE("        ulCapacity   : %d B, %.1f KB, %.1f MB\n", pxCtrl->xConfig.ulCapacity, (float)(pxCtrl->xConfig.ulCapacity) / 1024.f, (float)(pxCtrl->xConfig.ulCapacity) / 1024.f / 1024.f);
    TRACE("    hSpi             : %08X\n", pxCtrl->hSpi);
    TRACE("    phSpi            : %08X\n", pxCtrl->phSpi);
