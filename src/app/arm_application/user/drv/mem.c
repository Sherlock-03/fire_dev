...
status_t DrvMemInit(void)
{
    ...
    MemConfigCsPin(&xConfig, PORT(SPI1_CS), PIN(SPI1_CS));
...
#define SPIF_PAGE_SIZE 256
    ...
    cliprintf("    EraseWrite: %02X %02X %02X %02X %02X %02X %02X %02X\n", ucWtBuf[0], ucWtBuf[1], ucWtBuf[2], ucWtBuf[3], ucWtBuf[4], ucWtBuf[5], ucWtBuf[6], ucWtBuf[7]);
    MemRead(s_xMem, 0, 8, ucRdBuf);
    cliprintf("    Read      : %02X %02X %02X %02X %02X %02X %02X %02X\n", ucRdBuf[0], ucRdBuf[1], ucRdBuf[2], ucRdBuf[3], ucRdBuf[4], ucRdBuf[5], ucRdBuf[6], ucRdBuf[7]);
    ucWtBuf[0] = 0xAA;
    ucWtBuf[1] = 0xBB;
